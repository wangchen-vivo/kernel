...
// `heap` module (SpinLock wrappers) is kept in kernel/kernel/src/allocator/tlsf/heap.rs
// `int` module is in tlsf_int.rs
    ...
    Tlsf<'pool, FLBitmap, SLBitmap, FLLEN, SLLEN>
{
    ...
    pub unsafe fn insert_free_block_ptr_aligned(
        ...
        let pool_len = cursor.wrapping_sub(start);
        self.total += pool_len;

        NonZeroUsize::new(pool_len)
    }
    ...
    pub unsafe fn append_free_block_ptr(&mut self, block: NonNull<[u8]>) -> usize {
        ...
        start = start.wrapping_sub(GRANULARITY);
    ...
    #[inline]
    pub unsafe fn deallocate_block(&mut self, mut block: NonNull<BlockHdr>) -> usize {
...
/// Type alias for a TLSF heap with `usize` bitmaps and full bit-width.
